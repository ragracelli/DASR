...
# Adiciona pre-treino # Criado por R.A.G. ###################################################
parser.add_argument('--pretrain', default=0, help='Use Pre-Trained model')
#############################################################################################
...
def main(args):
    ...
    # Pre treino adic. por R.A.G. ############################################

    if args.pretrain:
        print('Using pre-trained model')
    #    #checkpoint = torch.load("pre_uaspeech.pth.tar")
        checkpoint = torch.load("final.pth.tar")
        model_state = checkpoint['state_dict']
        model.load_state_dict(model_state)
        model.eval() # adicionado para pre treino
    ...
    optimizier = TransformerOptimizer(
        ...
        #torch.optim.Adam(model.parameters(), betas=(0.9, 0.99), eps=1e-06),
        args.k,
        args.d_model,
        args.warmup_steps)
