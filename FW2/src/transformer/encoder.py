...
class Encoder(nn.Module):
    ...
    def __init__(self, d_input, n_layers, n_head, d_k, d_v,
        ...
        self.layer_norm_in = nn.LayerNorm(d_model, eps=1e-06)
        ...
        ultimo_encoder = self.layer_stack[-1]
        for param in ultimo_encoder.parameters():
            param.requires_grad = False
            print(param.requires_grad)

        penultimo_encoder = self.layer_stack[-2]
        for param in penultimo_encoder.parameters():
            param.requires_grad = False
            print(param.requires_grad)
    ...
    def forward(self, padded_input, input_lengths, return_attns=False):
        ...
        for enc_layer in self.layer_stack:
            enc_output, enc_slf_attn, enc_slf_attn2 = enc_layer(
                enc_output,
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_input, non_pad_mask=None, slf_attn_mask=None):
        enc_output1, enc_slf_attn = self.slf_attn(
            enc_input, enc_input, enc_input, mask=slf_attn_mask)
        enc_output1 *= non_pad_mask

        enc_output1 = self.pos_ffn(enc_output1)
        enc_output1 *= non_pad_mask

        # Passando a saída do primeiro bloco de atenção para o segundo
        enc_output, enc_slf_attn2 = self.slf_attn2(
            enc_output1, enc_output1, enc_output1, mask=slf_attn_mask)#add
        enc_output *= non_pad_mask #add

        enc_output = self.pos_ffn(enc_output) #add
        enc_output *= non_pad_mask #add

        return enc_output, enc_slf_attn, enc_slf_attn2 #add enc_slf_attn2
