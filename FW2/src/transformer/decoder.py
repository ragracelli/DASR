...
class Decoder(nn.Module):
    ...
    def __init__(
        ...
        mapping = torch.randint(n_tgt_vocab, (93,)) # adicionado
        self.tgt_word_emb = nn.Embedding(n_tgt_vocab, d_word_vec) # original
        #self.tgt_word_emb = nn.Embedding(n_tgt_vocab, d_word_vec)  # adicionado
        self.tgt_word_emb.weight.data = self.tgt_word_emb.weight.data[mapping]  # adicionado
        ...
        #        for param in layer.parameters():  # Adicionado
        #            param.requires_grad = False   # adicionado
        #            print(param.requires_grad)
                    #print(f"Gradiente de {param}: {param.grad}")

        # Para congelar o ultimo componente feed-foward conforme base de dados UA

        #ultima_camada = self.layer_stack[-1]
        #for param in ultima_camada.pos_ffn.parameters():
        #    param.requires_grad = False
        #    print(param.requires_grad)

        #    # Para congelar ultimo decoder

        #ultimo_decoder = self.layer_stack[-1]
        #for param in ultimo_decoder.parameters():
        #    param.requires_grad = False
        #    print(param.requires_grad)
...
######################## add ######################
            for hyp in hyps:
                print('hypo: ' + ''.join(
                    [char_list[int(x)] if int(x) < len(char_list) else '' for x in hyp['yseq'][0, 1:]]))
###################################################
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_input, enc_output, non_pad_mask=None, slf_attn_mask=None, dec_enc_attn_mask=None):
        ...
        dec_output = self.layernorm1(dec_output)
        ...
        dec_output = self.layernorm3(dec_output)
